...
export const ComparativeReports = ({ activities, statuses, getStatusColor }: ComparativeReportsProps) => {
  const [compareBy, setCompareBy] = useState<'equipe' | 'status' | 'cidade'>('equipe');
  ...
  const monthlyTrend = useMemo(() => {
    ...
    activities.forEach(a => {
      const key = compareBy === 'equipe' ? a.equipe : compareBy === 'status' ? a.status : a.cidade;
    ...
    return monthList.map(month => {
      ...
      topCategories.forEach(cat => {
        row[cat] = activities.filter(a => {
          ...
          const val = compareBy === 'equipe' ? a.equipe : compareBy === 'status' ? a.status : a.cidade;
  ...
  const COLORS = ['hsl(220, 70%, 50%)', 'hsl(142, 76%, 36%)', 'hsl(45, 93%, 47%)', 'hsl(0, 84%, 60%)', 'hsl(262, 83%, 58%)'];
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Select value={compareBy} onValueChange={(v: 'equipe' | 'status' | 'cidade') => setCompareBy(v)}>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={periodComparison} layout="vertical">
                ...
                <Tooltip />
                <Legend />
                <Bar dataKey="mesAtual" fill="hsl(220, 70%, 50%)" name="Mês Atual" radius={[0, 4, 4, 0]} />
                <Bar dataKey="mesAnterior" fill="hsl(220, 70%, 50%, 0.4)" name="Mês Anterior" radius={[0, 4, 4, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={completionEvolution}>
              ...
              <Legend />
              <Line type="monotone" dataKey="taxa" stroke="hsl(142, 76%, 36%)" strokeWidth={3} name="Taxa de Conclusão %" dot={{ r: 4 }} />
