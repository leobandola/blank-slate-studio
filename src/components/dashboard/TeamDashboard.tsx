...
export const TeamDashboard = ({ activities, osiActivities, statuses, getStatusColor }: TeamDashboardProps) => {
  const [selectedTeam, setSelectedTeam] = useState<string>('all');
  const [period, setPeriod] = useState<'7' | '14' | '30'>('14');

  const teams = useMemo(() => {
    const teamSet = new Set<string>();
    activities.forEach(a => { if (a.equipe) teamSet.add(a.equipe); });
  ...
  const filteredActivities = useMemo(() => {
    ...
    if (selectedTeam !== 'all') {
      filtered = filtered.filter(a => a.equipe === selectedTeam);
    }
    return filtered;
  ...
  const teamComparison = useMemo(() => {
    ...
    return teams.map(team => {
      const teamActs = periodActivities.filter(a => a.equipe === team);
  ...
  // Radar data for team performance
  const radarData = useMemo(() => {
    if (teamComparison.length === 0) return [];
    const maxTotal = Math.max(...teamComparison.map(t => t.total), 1);
    const maxCompleted = Math.max(...teamComparison.map(t => t.completed), 1);
  ...
  const teamTrend = useMemo(() => {
    ...
    for (let i = Number(period) - 1; i >= 0; i--) {
      ...
      topTeams.forEach(t => {
        dayData[t.name] = filteredActivities.filter(a => a.data === dateStr && a.equipe === t.name).length;
      });
      days.push(dayData);
    }
    return days;
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <RadarChart data={radarData}>
                ...
                <Radar name="Produtividade" dataKey="produtividade" stroke="hsl(220, 70%, 50%)" fill="hsl(220, 70%, 50%)" fillOpacity={0.3} />
